...
import { getLoggedInAccount, isURL, parseBaseURL } from '@/utils/auth.ts';
...
const AUTH_SESSION_KEY = 'soapbox:auth:me';
...
const getAuthUserFromAccount = (accessToken: string, account: any) => {
  const id = account?.id;
  const url = [account?.url, account?.pleroma?.ap_id].find(isURL);
...
const rememberVerifiedAccount = (accessToken: string, account: any) => {
  const authUser = getAuthUserFromAccount(accessToken, account);
...
const persistVerifiedAuthUser = (auth: RootState['auth'], authUser: { id: string; url: string; access_token: string }) => {
  /*
    Keep browser storage coherent after a successful verify_credentials call.
    Some Rebased account payloads are useful to the UI before every strict
    account schema accepts them, but the session only needs this small identity.
  */
  try {
    const nextAuth = {
      ...auth,
      ...
      me: auth.me && auth.users[auth.me] ? auth.me : authUser.url,
    };

    window.localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify(nextAuth));
    window.sessionStorage.setItem(AUTH_SESSION_KEY, nextAuth.me);
  } catch (_e) {
    // Storage can be unavailable or locked in hardened browser modes.
  }
};
...
export const verifyCredentials = (token: string, accountUrl?: string) => {
  ...
  return (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return baseClient(token, baseURL).get('/api/v1/accounts/verify_credentials').then((response) => response.json()).then((account) => {
      ...
      const rememberAction = rememberVerifiedAccount(token, account);
      if (rememberAction) {
        dispatch(rememberAction);
        persistVerifiedAuthUser(getState().auth, rememberAction.authUser);
      }
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return dispatch(revokeOAuthToken(params as Record<string, string>))
      .finally(() => {
        ...
        window.localStorage.removeItem('soapbox:external:app');
        window.localStorage.removeItem('soapbox:external:baseurl');
        window.localStorage.removeItem('soapbox:external:scopes');
