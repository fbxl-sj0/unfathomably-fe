...
export async function registerPushNotifications(api: MastodonClient, vapidKey: string) {
  ...
  const result = await getOrCreateSubscription(vapidKey);

  if (!result) {
    return;
  }

  const { subscription, created } = result;
...
async function getOrCreateSubscription(vapidKey: string): Promise<{ subscription: PushSubscription; created: boolean } | null> {
  ...
  const newSubscription = await createSubscription(vapidKey);
  return { subscription: newSubscription, created: true };
}
...
async function createSubscription(vapidKey: string): Promise<PushSubscription> {
  ...
  return registration.pushManager.subscribe({
    ...
    applicationServerKey: decodeBase64Url(vapidKey).buffer as ArrayBuffer,
  });
}
...
/** Return true when the browser has permission to create a new push subscription. */
