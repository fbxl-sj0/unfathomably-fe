...
const fetchAccountByUsername = (username: string, history?: History) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    if (features.accountLookup && fullyQualifiedAcct(username)) {
    ...
    } else if (features.accountByUsername && (me || !features.accountLookup)) {
...
const fullyQualifiedAcct = (username: string) => username.includes('@');
...
const fetchRelationships = (accountIds: string[]) =>
  async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    if (!isLoggedIn(getState) || !getAccessToken(state)) return null;

    const loadedRelationships = state.relationships;
