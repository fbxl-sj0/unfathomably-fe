...
const externalAuthorize = (instance: InstanceV1, baseURL: string) =>
  (dispatch: AppDispatch, _getState: () => RootState) => {
    ...
    return dispatch(createExternalApp(instance, baseURL)).then((app) => {
      ...
      window.localStorage.setItem('soapbox:external:app', JSON.stringify(app));
      window.localStorage.setItem('soapbox:external:baseurl', baseURL);
      window.localStorage.setItem('soapbox:external:scopes', scopes);
...
export const loginWithCode = (code: string) =>
  (dispatch: AppDispatch) => {
    const { client_id, client_secret, redirect_uri } = JSON.parse(window.localStorage.getItem('soapbox:external:app')!);
    const baseURL = window.localStorage.getItem('soapbox:external:baseurl')!;
    const scope   = window.localStorage.getItem('soapbox:external:scopes')!;
    ...
    return dispatch(obtainOAuthToken(params, baseURL))
      ...
      .then(() => window.localStorage.removeItem('soapbox:external:baseurl'))
