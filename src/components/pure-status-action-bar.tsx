...
const messages = defineMessages({
  ...
  reactionSmirk: { id: 'status.reactions.smirk', defaultMessage: 'Smirk' },
...
const PureStatusActionBar: React.FC<IPureStatusActionBar> = ({
  ...
  const quoteAllowed = status.pleroma?.quote_allowed ?? publicStatus;
  ...
  ?? (
    status.favourited && account
      ? { count: 1, me: status.account.id === account.id, name: '👍', url: undefined }
      : undefined
  );
  ...
  return (
    <HStack data-testid='status-action-bar'>
      ...
      >
        ...
        {features.ditto && (acceptsZaps || acceptsZapsCashu) && (
          <StatusActionButton
