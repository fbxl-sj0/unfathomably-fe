...
const messages = defineMessages({
  ...
  focusX: { id: 'upload_form.focus_x', defaultMessage: 'Horizontal focal point' },
  focusY: { id: 'upload_form.focus_y', defaultMessage: 'Vertical focal point' },
  ocr: { id: 'upload_form.ocr', defaultMessage: 'Recognize text' },
});
...
const Upload: React.FC<IUpload> = ({
  ...
  const [dirtyFocus, setDirtyFocus] = useState<{ x: number; y: number } | null>(null);
  ...
  const descriptionLengthLabel = descriptionLimit ? `${description.length}/${descriptionLimit}` : String(description.length);
  const focusX = media.meta.getIn(['focus', 'x']) as number | undefined;
  const focusY = media.meta.getIn(['focus', 'y']) as number | undefined;
  const activeFocusX = dirtyFocus?.x ?? focusX ?? 0;
  const activeFocusY = dirtyFocus?.y ?? focusY ?? 0;
  const x = ((activeFocusX /  2) + .5) * 100;
  const y = ((activeFocusY / -2) + .5) * 100;
  const mediaType = media.type;
  const mimeType = media.pleroma.get('mime_type') as string | undefined;
  const isMediaCover = mediaType === 'video' || mediaType === 'audio';
  const hasFocalPoint = typeof onFocusChange === 'function' && mediaType === 'image';

  const handleFocusChange = (axis: 'x' | 'y') => (event: React.ChangeEvent<HTMLInputElement>) => {
    const nextFocus = {
      x: axis === 'x' ? Number(event.target.value) : activeFocusX,
      y: axis === 'y' ? Number(event.target.value) : activeFocusY,
    };

    setDirtyFocus(nextFocus);
            ...
            {onDescriptionChange && (
              <div className={clsx('absolute inset-x-0 bottom-0 z-[2px] bg-gradient-to-b from-transparent via-gray-900/50 to-gray-900/80 p-2.5 opacity-0 transition-opacity duration-100 ease-linear', { 'opacity-100': active })}>
                <label>
                  ...
                  <div className='mt-1 flex flex-wrap items-center justify-between gap-2 text-xs text-white/80'>
                    <span>
                      {descriptionLengthLabel}
                    </span>

                    {onOcr && mediaType === 'image' && (
                      <button type='button' className='rounded bg-white/15 px-2 py-1 hover:bg-white/25' onClick={onOcr}>
                        {intl.formatMessage(messages.ocr)}
                      </button>
                    )}
                  </div>
                </label>
                ...
                {hasFocalPoint && (
                  <div className='mt-2 grid grid-cols-1 gap-1 text-xs text-white/80'>
                    <label>
                      {intl.formatMessage(messages.focusX)}
                      <input
                        type='range'
                        min='-1'
                        max='1'
                        step='0.01'
                        value={activeFocusX}
                        onChange={handleFocusChange('x')}
                        className='w-full'
                      />
                    </label>

                    <label>
                      {intl.formatMessage(messages.focusY)}
                      <input
                        type='range'
                        min='-1'
                        max='1'
