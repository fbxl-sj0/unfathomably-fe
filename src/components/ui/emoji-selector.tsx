...
const EmojiSelector: React.FC<IEmojiSelector> = ({
  ...
  /** Configured reactions lead the compact row; recent picks fill any spare slots. */
  const emojis = new Set([...allowedEmoji, ...frequentNative]);
  const compactEmojiLimit = Math.max(6, allowedEmoji.size);
  ...
  return (
    ...
    >
      {expanded ? (
        ...
        >
          {[...emojis].slice(0, compactEmojiLimit).map((emoji, i) => (
            <EmojiButton
              key={i}
              emoji={emoji}
              onClick={handleReact}
