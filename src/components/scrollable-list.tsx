...
/** Scroll position saved in window.sessionStorage. */
...
const ScrollableList = forwardRef<VirtuosoHandle, IScrollableList>(({
  ...
  const scrollData: SavedScrollPosition | null = useMemo(() => JSON.parse(window.sessionStorage.getItem(scrollDataKey)!), [scrollDataKey]);
  ...
  useEffect(() => {
    ...
    window.sessionStorage.removeItem(scrollDataKey);

    return () => {
      if (scrollKey) {
        const data: SavedScrollPosition = { index: topIndex.current, offset: topOffset.current };
        window.sessionStorage.setItem(scrollDataKey, JSON.stringify(data));
      }
