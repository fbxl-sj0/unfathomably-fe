...
describe('statuses reducer', () => {
  ...
  describe('STATUS_IMPORT', () => {
    it('parses the status as a Record', async () => {
      const { default: status } = await import('@/__fixtures__/pleroma-quote-post.json');
    ...
    it('fixes the order of mentions', async () => {
      const { default: status } = await import('@/__fixtures__/status-unordered-mentions.json');
      const action = { type: STATUS_IMPORT, status };
    ...
    it('preserves the quote', async () => {
      ...
      const { default: quotedQuotePost } = await import('@/__fixtures__/pleroma-quote-of-quote-post.json');
    ...
    it('fixes Pleroma attachments', async () => {
      const { default: status } = await import('@/__fixtures__/pleroma-status-with-attachments.json');
      const action = { type: STATUS_IMPORT, status };
    ...
    it('hides CWs', async () => {
      const { default: status } = await import('@/__fixtures__/status-cw.json');
      const action = { type: STATUS_IMPORT, status };
    ...
    it('preserves custom emoji content and metadata', async () => {
      const { default: status } = await import('@/__fixtures__/status-custom-emoji.json');
      const action = { type: STATUS_IMPORT, status };


      const result = reducer(undefined, action).get('AGm7uC9DaAIGUa4KYK');
      expect(result?.content).toContain(':ablobcathyper:');
      expect(result?.content).toContain(':blobcatphoto:');
      expect(result?.emojis.map(emoji => emoji.shortcode).toJS()).toEqual([
        'ablobcathyper',
        'ageblobcat',
        'blobcatphoto',
      ]);
    ...
    it('builds search_index', async () => {
      const { default: status } = await import('@/__fixtures__/status-with-poll.json');
      const action = { type: STATUS_IMPORT, status };
...
Promoting free speech, even for people and ideas you dislike`;
    ...
    it('builds search_index with mentions', async () => {
      const { default: status } = await import('@/__fixtures__/pleroma-status-reply-with-mentions.json');
      const action = { type: STATUS_IMPORT, status };
