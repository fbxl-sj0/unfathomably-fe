...
const hydrateDraft = (compose: Compose, draft: any) => {
  if (!draft || composeHasContent(compose)) {
    return compose;
  }

  return compose.withMutations(map => {
    if (typeof draft.text === 'string') map.set('text', draft.text);
    if (typeof draft.content_type === 'string') map.set('content_type', draft.content_type);
    if (typeof draft.group_id === 'string') map.set('group_id', draft.group_id);
    if (typeof draft.in_reply_to === 'string') map.set('in_reply_to', draft.in_reply_to);
    if (typeof draft.privacy === 'string') map.set('privacy', draft.privacy);
    if (typeof draft.quote === 'string') map.set('quote', draft.quote);
    if (typeof draft.spoiler_text === 'string') map.set('spoiler_text', draft.spoiler_text);
    if (typeof draft.sensitive === 'boolean') map.set('sensitive', draft.sensitive);
    if (typeof draft.spoiler === 'boolean') map.set('spoiler', draft.spoiler);

    if (Array.isArray(draft.to)) {
      map.set('to', ImmutableOrderedSet<string>(draft.to.filter((item: any) => typeof item === 'string')));
    }

    if (Array.isArray(draft.media_attachments)) {
      map.set(
        'media_attachments',
        ImmutableList(draft.media_attachments.map((media: APIEntity) => normalizeAttachment(media))),
      );
    }

    if (draft.poll) {
      map.set('poll', PollRecord(draft.poll).set(
        'options',
        ImmutableList(Array.isArray(draft.poll.options) ? draft.poll.options : ['', '']),
      ));
    }

    if (draft.schedule) {
      map.set('schedule', new Date(draft.schedule));
    }
...
export default function compose(state = initialState, action: ComposeAction | EventsAction | MeAction | SettingsAction | TimelineAction) {
  switch (action.type) {
    case COMPOSE_DRAFTS_HYDRATE:
      return hydrateDrafts(state, action.drafts);
