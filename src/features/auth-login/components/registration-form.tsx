...
import PasswordIndicator from '@/features/auth-login/components/password-indicator.tsx';
...
const messages = defineMessages({
  ...
  domain: { id: 'registration.fields.domain_label', defaultMessage: 'Domain' },
  ...
  birthday: { id: 'edit_profile.fields.birthday_label', defaultMessage: 'Birthday' },
...
const RegistrationForm: React.FC<IRegistrationForm> = ({ inviteToken }) => {
  ...
  const mustMeetPasswordRequirements = features.passwordRequirements && !features.nostrSignup;
  ...
  const updateParams = (map: any) => {
    setParams(params.merge(ImmutableMap(map)) as ImmutableMap<string, any>);
  };
  ...
  const onSubmit: React.FormEventHandler = () => {
    if (mustMeetPasswordRequirements && !hasValidPassword) {
      return;
    }
    ...
    <Form onSubmit={onSubmit} data-testid='registrations-open'>
      <fieldset disabled={isLoading} className='space-y-3'>
        <>
          ...
          {domains && (
            <FormGroup labelText={intl.formatMessage(messages.domain)}>
              <Select
          ...
          {!features.nostrSignup && (
            <>
              <FormGroup labelText={intl.formatMessage(messages.password)}>
                <Input
                  type='password'
                  name='password'
              ...
              <FormGroup
                labelText={intl.formatMessage(messages.confirm)}
          ...
          {birthdayRequired && (
            <FormGroup labelText={intl.formatMessage(messages.birthday)}>
              <BirthdayInput
          ...
          <FormActions>
            <Button type='submit' disabled={mustMeetPasswordRequirements && !hasValidPassword}>
