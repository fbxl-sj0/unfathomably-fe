...
const GroupMembers: React.FC<IGroupMembers> = (props) => {
  ...
  const { groupMembers: moderators, isFetching: isFetchingModerators } = useGroupMembers(groupId, GroupRoles.MODERATOR);
  ...
  const isLoading = isFetchingGroup || isFetchingOwners || isFetchingAdmins || isFetchingModerators || isFetchingUsers || isFetchingPending;
  ...
  const members = useMemo(() => [
    ...
    ...admins,
    ...users,
  ], [owners, moderators, admins, users]);

  const canPromoteToAdmin = features.groupsAdminMax
    ? members.filter((member) => member.role === GroupRoles.ADMIN || member.role === GroupRoles.MODERATOR).length < MAX_ADMIN_COUNT
    : true;
