...
const WrappedRoute: React.FC<IWrappedRoute> = ({
  ...
  const loginRedirect = () => {
    ...
    window.localStorage.setItem('soapbox:redirect_uri', actualUrl);
...
const FallbackError: React.FC<FallbackProps> = ({ error, resetErrorBoundary }) => {
  ...
  return (
    <FallbackLayout>
      <ErrorColumn error={error instanceof Error ? error : new Error(String(error))} onRetry={resetErrorBoundary} />
    </FallbackLayout>
  );
};
