...
import { fetchReports, fetchUsers } from '@/actions/admin.ts';
...
import withGroupLookup from '@/components/hoc/group-lookup-hoc.tsx';
...
import { usePendingPolicy } from '@/queries/policies.ts';
...
import ComposeDraftRecovery from './util/compose-draft-recovery.tsx';
import PushNotificationFocusBridge from './util/push-notification-focus-bridge.tsx';
...
const GroupTagsSlug = withGroupLookup(GroupTags);
const GroupTagTimelineSlug = withGroupLookup(GroupTagTimeline);
const GroupTimelineSlug = withGroupLookup(GroupTimeline);
const GroupMembersSlug = withGroupLookup(GroupMembers);
const GroupGallerySlug = withGroupLookup(GroupGallery);
const ManageGroupSlug = withGroupLookup(ManageGroup);
...
const SwitchingColumnsArea: React.FC<ISwitchingColumnsArea> = ({ children }) => {
  ...
  return (
    <>
      <ComposeDraftRecovery />
      <PushNotificationFocusBridge />
      ...
      */}
        ...
        {isLoggedIn && <WrappedRoute path='/catchup' page={DefaultPage} component={Catchup} content={children} />}
        ...
        {features.groupsRecommendations && <WrappedRoute path='/groups/popular' exact page={GroupsPendingPage} component={GroupsPopular} content={children} />}
        {features.groupsRecommendations && <WrappedRoute path='/groups/suggested' exact page={GroupsPendingPage} component={GroupsSuggested} content={children} />}
        ...
        {features.groups && <WrappedRoute path='/group/:groupSlug' exact page={GroupPage} component={GroupTimelineSlug} content={children} publicRoute />}
        ...
        {features.ditto && <WrappedRoute path='/wallet' page={DefaultPage} component={Wallet} content={children} exact />}
        {features.ditto && <WrappedRoute path='/wallet/relays' page={DefaultPage} component={WalletRelays} content={children} exact />}
        {features.ditto && <WrappedRoute path='/wallet/mints' page={DefaultPage} component={WalletMints} content={children} exact />}
        {features.ditto && <WrappedRoute path='/wallet/transactions' page={DefaultPage} component={WalletTransactions} content={children} exact />}
        ...
        {features.nostrSignup && <WrappedRoute path='/login/nostr' page={DefaultPage} component={NostrBunkerLogin} publicRoute exact />}
...
const UI: React.FC<IUI> = ({ children }) => {
  ...
  const promptedPolicyId = useRef<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (!accountId || !policyId || promptedPolicyId.current === policyId) {
      return;
    }

    promptedPolicyId.current = policyId;
    dispatch(openModal('POLICY'));
  }, [accountId, dispatch, pendingPolicy?.pending_policy_id]);
