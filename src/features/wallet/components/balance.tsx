...
const NewMint = ({ onBack, list }: NewMintProps) => {
  ...
  const [quote, setQuote] = useState<Quote | undefined>(() => {
    const storedQuote = window.localStorage.getItem('soapbox:wallet:quote');
  ...
  const handleClean = useCallback(() => {
    ...
    window.localStorage.removeItem('soapbox:wallet:quote');
  }, []);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!quote) {
      try {
        ...
        window.localStorage.setItem('soapbox:wallet:quote', JSON.stringify(newQuote));
        setQuote(newQuote);
      ...
      <Stack space={6}>
        {!quote ? <Stack space={2}>
          ...
          : <Stack space={3} justifyContent='center' alignItems='center'>
            ...
            <QRCodeSVG className='rounded-lg' value={quote.request} includeMargin />
