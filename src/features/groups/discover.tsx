...
const messages = defineMessages({
  placeholder: { id: 'groups.discover.search.placeholder', defaultMessage: 'Search or paste group@server' },
});
...
const Discover: React.FC = () => {
  ...
  } else if (features.groupsRecommendations) {
    content = (
      <>
        <PopularGroups />
        <SuggestedGroups />
        {features.groupsTags ? <PopularTags /> : null}
      </>
    );
  } else {
    content = (
      <Stack space={2} className='py-6'>
        <Text size='2xl' weight='bold'>
          <FormattedMessage
            id='groups.discover.remote.title'
            defaultMessage='Find a remote group'
          />
        </Text>

        <Text theme='muted'>
          <FormattedMessage
            id='groups.discover.remote.subtitle'
            defaultMessage='Search for a community handle like 3dprinting@lemmy.world to fetch it, open it, and join it.'
          />
        </Text>
      </Stack>
    );
