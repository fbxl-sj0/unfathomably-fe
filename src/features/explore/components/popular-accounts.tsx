...
const AccountsCarousel = () => {
  ...
  const handleClick = () => {
    setIsOpen((prev) => {
      const newValue = !prev;
      window.localStorage.setItem('soapbox:explore:accounts:status', JSON.stringify(newValue));
      return newValue;
    });
  };
  ...
  useEffect(
    () => {
      const isOpenStatus = window.localStorage.getItem('soapbox:explore:accounts:status');
      if (isOpenStatus) {
