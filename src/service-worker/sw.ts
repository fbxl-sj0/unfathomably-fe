...
import { locales } from './web-push-locales.compile.ts';
...
/** Unfathomably FE notification data from push event. */
...
const notify = (options: ExtendedNotificationOptions): Promise<void> =>
  self.registration.getNotifications().then(notifications => {
    const alreadyVisible = notifications.some(notification => notification.tag === options.tag);
...
const hasVisibleClient = () =>
  self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clients =>
    clients.some(client => client.visibilityState === 'visible' && new URL(client.url).origin === self.location.origin),
  );
...
const handlePush = (event: PushEvent) => {
  ...
  event.waitUntil(
    fetchFromApi(`/api/v1/notifications/${notification_id}`, 'get', access_token).then(notification => {
      if (!DISPLAYABLE_TYPES.includes(notification.type)) {
        return;
      }
      ...
      return hasVisibleClient().then((visible) => {
        if (visible) {
          return;
        }
...
/** Callback for focused app tabs asking us to quiet stale native notifications. */
const handleMessage = (event: ExtendableMessageEvent) => {
  if (event.data?.type !== 'notifications:close_visible') {
    return;
  }
