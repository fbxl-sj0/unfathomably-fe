...
const DEFAULT_ALLOWED_EMOJI = ImmutableList<string>([
  '\u{1f44d}',
  '\u{2764}\u{fe0f}',
  '\u{1f914}',
  '\u{1f606}',
...
export const SoapboxConfigRecord = ImmutableRecord({
  ...
  allowedEmoji: DEFAULT_ALLOWED_EMOJI,
  verifiedIcon: '',
...
const normalizeCryptoAddress = (address: unknown): CryptoAddress => {
  return CryptoAddressRecord(fromJS(address || {}) as any).update('ticker', ticker => {
...
const normalizeRedirectRootNoLogin = (soapboxConfig: SoapboxConfigMap): SoapboxConfigMap => {
  ...
  try {
    ...
    console.error('You have configured an invalid redirect in Unfathomably config.');
    console.error(e);
    return soapboxConfig.delete('redirectRootNoLogin');
  }
};
