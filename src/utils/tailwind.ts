...
export const toTailwind = (soapboxConfig: SoapboxConfig): SoapboxConfig => {
  const colors = ImmutableMap(soapboxConfig.get('colors') as any) as SoapboxColors;
  const legacyColors = ImmutableMap(fromJS(fromLegacyColors(soapboxConfig)) as any) as SoapboxColors;
